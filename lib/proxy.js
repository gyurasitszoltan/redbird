    ...
    _this._getTarget(src, req).bind(_this).then(function (target) {
      if (target){
        if (shouldRedirectToHttps(_this.certs, src, target, _this)) {
          redirectToHttps(req, res, target, opts.ssl, opts.redirectType, log);
        } else {
...
//
function redirectToHttps(req, res, target, ssl, redirectType, log) {
  ...
  //
  res.writeHead(redirectType, { Location: url });
  res.end();
}
